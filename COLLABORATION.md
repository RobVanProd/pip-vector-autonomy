...
- Latency work: added Ollama metrics to `PlanResponse`, `/llm/warmup`, `/diagnostics/latency-sample`, plus extended harness/analyzer scripts. Calibration found 16s Gemma reload spikes after vision/map calls; fixed by separating `VECTOR_MAP_VISION_MODEL` from validation vision and rewarming Gemma after vision-heavy calls. Post-fix 3-minute calibration averaged ~2.0s planner samples with no errors.
- A 3-hour harness was started at `vector/logs/extended-harness-20260515-210034-long`; analyze it with `scripts/analyze-extended-harness.ps1` when complete.
